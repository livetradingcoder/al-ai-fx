//+------------------------------------------------------------------+
//|                            AL-ai-FX Gold Breakout Aggressive.mq5 |
//|                                                         AL-ai-FX |
//|                                             https://al-ai-fx.xyz |
//+------------------------------------------------------------------+
...
#property description "Gold Breakout Aggressive - one session-range breakout a day on XAUUSD with a heavy hedge"
...
//--- User Inputs
input string BS = "---------AL-ai-FX Gold Breakout Aggressive---------";
...
const int MagicNumber = 20260511;
const string comm = "AL-ai-FX BreakoutA";   // MUST NOT contain "_"
const double StopLossFB = 15.0;
const double TakeProfitFB = 5.0;
const double Buffer = 0.16;
const double StopLossFS = 15.0;
const double TakeProfitFS = 5.0;
const double TakeProfitST = 1.5;
...
bool IsLicenceExpired() {
  ...
  if (!announced) {
    announced = true;
    Print("BreakoutA: Licence expired - no new trades. Open positions keep their stops.");
    Comment("AL-ai-FX Gold Breakout Aggressive: licence expired - renew at al-ai-fx.xyz");
  }
  return true;
}
...
int OnInit() {
  ...
  if (AccountProtectON) {
    for (int i = 0; i < ArraySize(allowed_accounts); i++) {
      if (account == allowed_accounts[i]) {
        IsAccount = true;
        Print("BreakoutA: Account verified.");
        break;
      }
    }
  } else {
  ...
  if (ExpiredON && IsExpired) {
    Print("BreakoutA: EA has expired.");
    return (INIT_FAILED);
  }

  if (AccountProtectON && !IsAccount) {
    Print("BreakoutA: Unauthorized account.");
    return (INIT_FAILED);
  }

  trade.SetExpertMagicNumber(MagicNumber);
  Print("BreakoutA: Initialized successfully.");
