//+------------------------------------------------------------------+
//|                         AL-ai-FX Gold MultiRange 6 Aggressive.mq5 |
//|                                                         AL-ai-FX |
//|                                             https://al-ai-fx.xyz |
//+------------------------------------------------------------------+
...
#property description "Gold MultiRange 6 Aggressive - six independent session-range breakouts on XAUUSD, each with its own hedge"
...
input int MagicNumber = 20260601;         // Magic Number Base (1=base,2=+1,3=+2,4=+3,5=+4,6=+5,7=+6)
// Hidden: an underscore typed into an input here would break the "_1_R" /
// "_2_R" parsing the EA uses to recognise its own orders.
string comm = "AL-ai-FX MR6";             // Trade Comment base - MUST NOT contain "_"

input string SR = "---------RANGE ON/OFF---------";
input bool Range1Enabled = true;  // Range 1 - Enable
input bool Range2Enabled = true;  // Range 2 - Enable
input bool Range3Enabled = true;  // Range 3 - Enable
...
// Range 2 time (hidden) - Breakout Aggressive
int TimeStart2 = 7;          int TimeStartMinute2 = 0;
int TimeStop2  = 10;         int TimeStopMinute2  = 0;

// Range 3 time (hidden) - Breakout Conservative
...
// Range 2 params - Breakout Aggressive (TP 5, SL 15, x15, hedge TP 1.5)
double StopLossFB2 = 15.0;   double TakeProfitFB2 = 5.0;   double Buffer2 = 0.16;
double StopLossFS2 = 15.0;   double TakeProfitFS2 = 5.0;
double Multiplier2 = 15.0;   double TakeProfitST2 = 1.5;

// Range 3 params - Breakout Conservative (TP 2, SL 13.3, x5, hedge TP 2)
double StopLossFB3 = 13.3;   double TakeProfitFB3 = 2.0;   double Buffer3 = 0.16;
double StopLossFS3 = 13.3;   double TakeProfitFS3 = 2.0;
...
bool IsLicenceExpired() {
  ...
  if (!announced) {
    announced = true;
    Print("MR6: Licence expired - no new trades. Open positions are still managed.");
    Comment("AL-ai-FX Gold MultiRange 6 Aggressive: licence expired - renew at al-ai-fx.xyz");
  }
  return true;
}
...
int OnInit() {
  ...
  if (AccountProtectON) {
    for (int i = 0; i < ArraySize(allowed_accounts); i++) {
      if (account == allowed_accounts[i]) { IsAccount = true; Print("MR6: Account verified."); break; }
    }
  } else { IsAccount = true; }

  if (TimeCurrent() <= ExpiredTime) IsExpired = false; else IsExpired = true;

  if (ExpiredON && IsExpired)        { Print("MR6: EA has expired.");       return (INIT_FAILED); }
  if (AccountProtectON && !IsAccount){ Print("MR6: Unauthorized account."); return (INIT_FAILED); }
  ...
  for (int r = 1; r <= 7; r++) {
    if (IsRangeAlreadyOver(r)) {
      ...
      Print("MR6: R",r," - Started after TimeStop, blocked for today.");
    }
  }
...
void DeletePrimaryOrderForRange(int range, const ENUM_ORDER_TYPE type) {
  ...
  for (int i=OrdersTotal()-1; i>=0; i--) {
    ...
    if (OrderSelect(t) && OrderGetInteger(ORDER_MAGIC)==magic &&
      ...
      if (k>=2 && (int)StringToInteger(cm[1])==1) { trade.OrderDelete(t); Print("MR6: R",range," - Deleted primary ",t); }
    }
  }
}
...
void ExecuteMarketBuy(double price, string comment, int range, double highRange) {
  ...
  if (trade.Buy(BuyLot, NULL, price, BuySL, BuyTP, comment)) {
    ...
    Print("MR6: R",range," - Market Buy, Hedge SellStop @ ",HedgeEntry," (actual SL)");
  }
}
...
void ExecuteMarketSell(double price, string comment, int range, double lowRange) {
  ...
  if (trade.Sell(SellLot, NULL, price, SellSL, SellTP, comment)) {
    ...
    Print("MR6: R",range," - Market Sell, Hedge BuyStop @ ",HedgeEntry," (actual SL)");
  }
}
...
void ProcessRangeLogic(int range, double highRange, double lowRange, double askPrice, double bidPrice, double minDistance) {
  ...
  if (!GetBuyPending(range) && !GetTradeTaken(range)) {
    ...
    if (askPrice >= BuyStopEntry) {
      Print("MR6: R",range," - Price above Buy level - MARKET EXECUTION");
      ExecuteMarketBuy(askPrice, CMPrimary, range, highRange);
    }
    else if (dist < minDistance && dist > 0) {
      Print("MR6: R",range," - Buy too close (",DoubleToString(dist,2)," < ",DoubleToString(minDistance,2),") - monitoring");
    }
      ...
      if (trade.BuyStop(BuyLot, BuyStopEntry, NULL, BuySL, BuyTP, ORDER_TIME_DAY, 0, CMPrimary)) {
        ...
        Print("MR6: R",range," - BuyStop @ ",BuyStopEntry);
      }
    }
  }
  ...
  if (!GetSellPending(range) && !GetTradeTaken(range)) {
    ...
    if (bidPrice <= SellStopEntry) {
      Print("MR6: R",range," - Price below Sell level - MARKET EXECUTION");
      ExecuteMarketSell(bidPrice, CMPrimary, range, lowRange);
    }
    else if (dist < minDistance && dist > 0) {
      Print("MR6: R",range," - Sell too close (",DoubleToString(dist,2)," < ",DoubleToString(minDistance,2),") - monitoring");
    }
    else if (dist >= minDistance) {
      ...
      if (trade.SellStop(SellLot, SellStopEntry, NULL, SellSL, SellTP, ORDER_TIME_DAY, 0, CMPrimary)) {
        ...
        Print("MR6: R",range," - SellStop @ ",SellStopEntry);
      }
    }
  }
}
...
void OnTradeTransaction(const MqlTradeTransaction& trans,
  ...
  if (StringFind(dealComment, "_1_") >= 0) {
    ...
    if (dealType == DEAL_TYPE_BUY) {
      Print("MR6: R",range," - BuyStop filled @ ",dealPrice);
      DeletePrimaryOrderForRange(range, ORDER_TYPE_SELL_STOP);
      ...
      trade.SetExpertMagicNumber(GetMagicNumber(range));
      trade.SellStop(HedgeLot, HedgeEntry, NULL, HedgeSL, HedgeTP, ORDER_TIME_GTC, 0, CMHedge);
      Print("MR6: R",range," - Hedge SellStop @ ",HedgeEntry," (actual SL)");
    }
    // Primary SELL filled
    else if (dealType == DEAL_TYPE_SELL) {
      Print("MR6: R",range," - SellStop filled @ ",dealPrice);
      DeletePrimaryOrderForRange(range, ORDER_TYPE_BUY_STOP);
      ...
      trade.SetExpertMagicNumber(GetMagicNumber(range));
      trade.BuyStop(HedgeLot, HedgeEntry, NULL, HedgeSL, HedgeTP, ORDER_TIME_GTC, 0, CMHedge);
      Print("MR6: R",range," - Hedge BuyStop @ ",HedgeEntry," (actual SL)");
    }
  }
}
