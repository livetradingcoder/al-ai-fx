...
Phase: 6 of 7 (Public Catalog, Per-Robot Pricing, Free Trials) — **IN PROGRESS (1/4 plans)**
Status: **Phase 6 Plan 1 (06-01) COMPLETE.** Per-robot pricing data foundation landed: `RobotPrice` table (price-only — composite unique `(robotId, tier)`, `amount`, `active`; TIER_METADATA stays SSoT for durations/priceStrings), a Postgres **partial unique index** `Subscription_one_free_trial_per_robot ... WHERE tier = 'FREE_TRIAL'` (DB-enforced one-free-trial-per-robot existence-ever; applied as raw SQL, intentionally invisible to `migrate diff` — must NOT be reverted), the fail-closed `resolveRobotPrice(robotSlug, tierRaw)` server-side price authority (reuses `mapTier`; throws `UnknownRobotError`/`UnknownRobotPriceError`; amount always server-computed, never client-trusted), and an idempotent GoldBot seed (8 tiers from TIER_METADATA, `update:{}` so 06-04 admin edits are never clobbered). Applied to LIVE PRODUCTION via the incremental build-step `migrate deploy` channel (NOT reset) — build log confirmed "Applying migration 20260706_add_robot_pricing_and_trial_index / All migrations successfully applied" + "[seed-robot-prices] Seeded 8 RobotPrice rows for goldbot (id=cmr86vu530000jf5xbsggcfrv)"; temp build-step reverted in a separate commit. tsc + eslint clean. **Wave 2 unblocked:** 06-02 (catalog RSC page), 06-03 (robotSlug funnel + HMAC + trial P2002 handling), 06-04 (admin price editor) all depend on this plan and can now run **in parallel**. Downstream: resolve all prices through `resolveRobotPrice`, composite key is `robotId_tier`, the trial index throws P2002 on a 2nd FREE_TRIAL insert for the same (userId, robotId).
...
Progress: [███████████████░░░░░░░░░░░] 61% (17/28 plans across all phases; 4/4 Phase 1, 3/3 Phase 2, 3/3 Phase 3, 3/3 Phase 4, 4/4 Phase 5, 1/4 Phase 6)
